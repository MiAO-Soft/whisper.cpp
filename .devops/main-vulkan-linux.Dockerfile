...
# Download vulkan
RUN curl -o vulkansdk.tar.xz https://sdk.lunarg.com/sdk/download/1.4.335.0/linux/vulkansdk-linux-x86_64-1.4.335.0.tar.xz && \
    tar -xvf vulkansdk.tar.xz && \
    source vulkansdk/1.4.335.0/setup-env.sh

RUN apt-get update && \
    apt-get install -y build-essential libsdl2-dev wget cmake git libsdl2-dev && \
    apt-get clean && \
